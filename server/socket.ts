...
// const fs = require('fs')
// const hostkeys = JSON.parse(fs.readFileSync('./hostkeyhashes.json', 'utf8'))
...
module.exports = function socket (socket) {
  ...
  conn.on('ready', function connOnReady () {
    ...
    conn.shell({
      ...
      if (socket.request.session.ssh.serverlog.client) const dataBuffer
  ...
  if (socket.request.session.username && socket.request.session.userpassword && socket.request.session.ssh) {
    ...
    debugWebSSH2('Attempt to connect without session.username/password or session constialbles defined, potentially previously abandoned client session. disconnecting websocket client.\r\nHandshake information: \r\n  ' + JSON.stringify(socket.handshake));
  ...
  /**
  * Error handling for constious events. Outputs error to client, logs to
